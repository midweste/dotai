...
class TestMemoryStore:
    ...
    def test_query_by_file_exact(self, memory_store):
        ...
        memory_store.create(Memory(
            summary="Foo service", type="pattern", files=["src/services/foo.py"],
        ))
        memory_store.create(Memory(
            summary="Bar service", type="pattern", files=["src/services/bar.py"],
        ))
    ...
    def test_query_by_file_directory(self, memory_store):
        ...
        memory_store.create(Memory(
            summary="Service A", type="pattern", files=["src/services/a.py"],
        ))
        memory_store.create(Memory(
            summary="Service B", type="pattern", files=["src/services/b.py"],
        ))
        memory_store.create(Memory(
            summary="Model C", type="pattern", files=["src/models/c.py"],
        ))
    ...
    def test_query_by_file_min_importance(self, memory_store):
        ...
        memory_store.create(Memory(
            summary="Important", type="decision", importance=0.9,
            files=["src/app.py"],
        ))
        memory_store.create(Memory(
            summary="Unimportant", type="context", importance=0.1,
            files=["src/app.py"],
        ))
        results = memory_store.query_by_file("src/app.py", min_importance=0.5)
        assert len(results) == 1
    ...
    def test_list_all(self, memory_store):
        ...
        memory_store.create(Memory(summary="Low", type="context", importance=0.2))
        memory_store.create(Memory(summary="High", type="context", importance=0.9))
...
class TestBuildMetaStore:
    def test_record_and_get(self, build_meta_store):
        ...
        entry = build_meta_store.record(BuildMetaEntry(
            build_type="incremental", last_commit="abc123",
            commit_count=10, memory_count=5,
        ))
        assert entry.id is not None
        last = build_meta_store.get_last()
        assert last.last_commit == "abc123"
    ...
    def test_get_last_returns_newest(self, build_meta_store):
        ...
        build_meta_store.record(BuildMetaEntry(
            build_type="full", last_commit="aaa", commit_count=5, memory_count=2,
        ))
        build_meta_store.record(BuildMetaEntry(
            build_type="incremental", last_commit="bbb", commit_count=3, memory_count=4,
        ))
        last = build_meta_store.get_last()
        assert last.last_commit == "bbb"

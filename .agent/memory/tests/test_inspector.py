...
from src.models import Memory, BuildMetaEntry
...
class TestInspector:
    ...
    def test_single_memory_not_found(self, inspector):
        ...
        result = inspector.inspect("memory 999")
    ...
    def test_builds_with_data(self, inspector, stores):
        ...
        stores["build_meta_store"].record(BuildMetaEntry(
            build_type="full", last_commit="abc", commit_count=10, memory_count=5,
        ))
        result = json.loads(inspector.inspect("builds"))
        assert len(result) == 1
        assert result[0]["last_commit"] == "abc"
